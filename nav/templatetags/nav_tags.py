...
class RootNavigationNode(template.Node):
  ...
  def render(self, context):
    items = [k for k in Item.objects.filter(parent=None) if k.is_allowed(user)]
...
def get_root_items(parser, token):
  ...
  m = re.search(r'^(\w+) as (\w+)$', arg.strip())
  if not m:
    raise template.TemplateSyntaxError, \
  ...
  try:
    ...
    raise template.TemplateSyntaxError, "%s is an invalid username" % username
